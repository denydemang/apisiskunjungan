...
class LocationCheckController extends Controller
{
    ...
    private const LOCAL_ISP_WHITELIST = [
        'telkom', 'indosat', 'xl axiata', 'tri', 'smartfren'
    ];
    ...
    public function check(LocationCheckRequest $request)
    {
        ...
        try {
            ...
            // Deteksi VPN awal
            $org = $ipData['org'] ?? '';
            $isVpn = $this->isVpnConnection($org);

            // Pengecualian: Jika provider lokal dan jarak masih wajar (<= 100km), jangan anggap VPN
            $orgLower = strtolower($org);
            $isWhitelisted = false;

            foreach (self::LOCAL_ISP_WHITELIST as $safeProvider) {
                if (str_contains($orgLower, $safeProvider)) {
                    $isWhitelisted = true;
                    break;
                }
            }

            if ($isWhitelisted && $distanceKm <= 80) {
                $isVpn = false;
            }

            // Logging tambahan untuk debug
            Log::info("ISP: $orgLower | Jarak: $distanceKm km | Whitelisted: " . ($isWhitelisted ? 'Yes' : 'No') . " | VPN: " . ($isVpn ? 'Yes' : 'No'));
    ...
    private function calculateDistance(float $lat1, float $lon1, float $lat2, float $lon2): float
    {
        ...
        return $coord1->getDistance($coord2, new Vincenty()) / 1000;
    }
